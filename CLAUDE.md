...
**DXLink streaming worker (`dxlink-worker/`):** Long-running Deno process on Fly.io. Logs into Tastytrade, exchanges the session token for a DXLink streamer token via `/api-quote-tokens`, opens one WebSocket to `tasty-openapi-ws.dxfeed.com/realtime`, and subscribes to: equity `Quote` for ~15 curated tickers (SPY/QQQ/IWM/AAPL/NVDA/TSLA/MSFT/AMD/AMZN/META/GOOGL/LLY/NVO/MRK/PFE), and per-option `Quote` + `Greeks` + `Summary` for the front 2 expirations within ATM ± 25%. Every event lands in an in-memory shadow Map keyed by streamer symbol; a 750ms flush loop upserts dirty rows into `public.dxlink_quotes`. Reconnects with exponential backoff (max 60s), refreshes the streamer token at ~22h, refreshes the chain plan every 4h to pick up new strikes/expirations. Memory ~150MB, CPU near-zero outside market hours. **Deploy:** see `dxlink-worker/README.md` — `fly launch` + `fly secrets set TASTYTRADE_USERNAME=… TASTYTRADE_PASSWORD=… SUPABASE_URL=… SUPABASE_SERVICE_ROLE_KEY=…` + `fly deploy`. The worker MUST run on Tastytrade production base URL (`api.tastyworks.com`) — sandbox DXLink delivers mock data only.
