...
- Components from the original spec not built (Kalshi-flow only): `KalshiMarketPanel`, `CombinedPnlStats`
- Optional follow-up: extend the `analyze-signal` Claude prompt with a `strike_suggestion` block (buy/sell %OTM, expected move %, max premium %); requires redeploying the edge function and re-running prompt regression. The calculator works fine without it.
...
*Status: Weeks 1–9 deployed — app feature-complete including the StrikePriceCalculator (Signal Detail + Log Signal step 2 + standalone `/calculator`). Outstanding: PWA icon binaries, Kalshi flow (`kalshi-analysis` edge function + Kalshi UI), server-side push delivery, auto-trigger of the -50% stop loss.*
