...
- `get-account` v1 (`verify_jwt=true`). Lists Tastytrade accounts the bot has access to with balances. Same Tastytrade auth helper as `place-order`. **Tastytrade requires you to add a customer profile in the sandbox dashboard before any account endpoints return data** — see `https://developer.tastytrade.com/sandbox/`. Auth helper caches the session token in `tastytrade_sessions` for ~23h and re-logs in on 401 or expiry.
