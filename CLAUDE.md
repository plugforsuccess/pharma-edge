...
**Scraper:** Python 3.11 in `scraper/`. Two passes per cron run:
- **Broad pass** — ClinicalTrials.gov v2 (Phase 2/3 readouts in next 120 days), FDA press-release RSS, SEC EDGAR full-text search (8-K + S-3). Scores trials 0–10 on phase, enrollment size, endpoint count, site terminations, protocol amendments. Top 5 scored candidates pass through Claude Sonnet 4.6 for preliminary thesis + flags, then land in `scanner_candidates` with `requested_by IS NULL` (visible to all authenticated users).
- **Watchlist pass** — `scrapers/watchlist.py` reads every `watchlist` row across all users, queries CT.gov / SEC EDGAR / FDA RSS for that ticker + company, dedups against existing candidates, and inserts new findings as personal candidates with `requested_by = user_id` (RLS-scoped to that user only). Idempotent across cron runs.
