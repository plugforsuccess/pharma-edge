...
// ─── Market metrics (IV + HV + IV rank/percentile) ─────────────────
// /market-metrics returns rich vol data for a symbol; we read the
// implied/historical volatility fields and rank/percentile so Claude
// can calibrate strike_suggestion to actual market conditions instead
// of training-data priors.

export interface MarketMetrics {
  ticker: string
  iv: number | null               // 30-day IV, decimal (0.45 = 45%)
  ivRank: number | null           // 0..1
  ivPercentile: number | null     // 0..1
  hv30: number | null
  ivHvDifference: number | null
  beta: number | null
}
...
export async function fetchMarketMetrics(
  ...
  try {
    resp = await tastytradeFetch(
      supabase,
      `/market-metrics?symbols=${encodeURIComponent(ticker)}`,
      { method: 'GET' },
    )
  } catch (_err) {
    return null
  ...
  return {
    ticker,
    iv: num(item['implied-volatility-index']) ?? num(item['implied-volatility-30-day']),
    ivRank: num(item['implied-volatility-index-rank']),
    ivPercentile: num(item['implied-volatility-percentile']),
    hv30: num(item['historical-volatility-30-day']),
    ivHvDifference: num(item['iv-hv-30-day-difference']),
    beta: num(item['beta']),
  }
}
